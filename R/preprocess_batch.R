...
preprocess_batch <- function(work_str, pattern=NULL, num_cores=1, ...) {

    in_dir <- file.path(work_str, 'in')

    if (!is.character(in_dir)) {
        stop('work_str needs to be of class character')
    }

    files_list_str <- list.files(path=in_dir, pattern=pattern, full.names=TRUE)


    mclapply(X=files_list_str, FUN=preprocess, work_str = work_str, num_cores=num_cores, ...)
